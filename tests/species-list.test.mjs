...
test('polyplacophoran artwork uses the shell icon instead of the fallback leaf', () => {
  assert.match(classIconSource, /Polyplacophora:\s*Shell/);
});

test('scaphopod artwork uses the shell icon instead of the fallback leaf', () => {
  assert.match(classIconSource, /Scaphopoda:\s*Shell/);
});

test('polychaete artwork uses the waves icon instead of the fallback leaf', () => {
  assert.match(classIconSource, /Polychaeta:\s*Waves/);
});

test('echinoid artwork uses the waves icon instead of the fallback leaf', () => {
  assert.match(classIconSource, /Echinoidea:\s*Waves/);
});

test('crinoid artwork uses the waves icon instead of the fallback leaf', () => {
  assert.match(classIconSource, /Crinoidea:\s*Waves/);
});
