import type {
  ConservationStatus,
  IucnStatusCode,
  PopulationTrend,
  Species,
  SpeciesSource,
  SpeciesTaxonomy,
  Taxon,
} from '../types/species';

const CONTENT_DATE = '2026-08-02' as const;

const iucnLabels: Record<IucnStatusCode, { zh: string; en: string }> = {
  EX: { zh: '灭绝', en: 'Extinct' },
  EW: { zh: '野外灭绝', en: 'Extinct in the Wild' },
  CR: { zh: '极危', en: 'Critically Endangered' },
  EN: { zh: '濒危', en: 'Endangered' },
  VU: { zh: '易危', en: 'Vulnerable' },
  NT: { zh: '近危', en: 'Near Threatened' },
  LC: { zh: '无危', en: 'Least Concern' },
  DD: { zh: '数据缺乏', en: 'Data Deficient' },
  NE: { zh: '未评估', en: 'Not Evaluated' },
};

const taxon = (scientificName: string, zhName: string): Taxon => ({
  scientificName,
  zhName,
});

const animalTaxonomy = (
  phylum: Taxon,
  className: Taxon,
  order: Taxon,
  family: Taxon,
  genus: Taxon,
): SpeciesTaxonomy => ({
  kingdom: taxon('Animalia', '动物界'),
  phylum,
  class: className,
  order,
  family,
  genus,
});

const conservation = (
  code: IucnStatusCode,
  trend: PopulationTrend,
  assessedYear: number,
  criteria?: string,
): ConservationStatus => ({
  system: 'IUCN Red List',
  code,
  label: iucnLabels[code],
  trend,
  assessedYear,
  criteria,
  assessor: 'International Union for Conservation of Nature (IUCN)',
});

const sourcesFor = (scientificName: string): readonly SpeciesSource[] => [
  {
    title: `IUCN Red List — ${scientificName}`,
    url: `https://www.iucnredlist.org/search?query=${encodeURIComponent(scientificName)}&searchType=species`,
    kind: 'conservation',
    accessedAt: CONTENT_DATE,
  },
  {
    title: `GBIF Backbone Taxonomy — ${scientificName}`,
    url: `https://www.gbif.org/species/search?q=${encodeURIComponent(scientificName)}`,
    kind: 'taxonomy',
    accessedAt: CONTENT_DATE,
  },
];

/**
 * Starter catalogue. Values are deliberately normalized for filters while
 * human-readable qualifiers remain in `measurements` and `featuredStats`.
 */
export const species = [
  {
    id: 'species-ailuropoda-melanoleuca',
    slug: 'giant-panda',
    names: { zh: '大熊猫', en: 'Giant Panda', aliases: ['猫熊', '竹熊'] },
    scientificName: 'Ailuropoda melanoleuca',
    taxonomy: animalTaxonomy(
      taxon('Chordata', '脊索动物门'),
      taxon('Mammalia', '哺乳纲'),
      taxon('Carnivora', '食肉目'),
      taxon('Ursidae', '熊科'),
      taxon('Ailuropoda', '大熊猫属'),
    ),
    conservation: conservation('VU', 'increasing', 2016, 'A2c; C2a(i)'),
    distribution: {
      realms: ['terrestrial'],
      continents: ['亚洲'],
      regions: ['东亚', '横断山—秦岭山地'],
      countries: ['中国'],
      endemicTo: ['中国'],
      range: '野生种群仅见于四川、陕西和甘肃相互隔离的山系。',
      center: { lat: 31.3, lng: 103.2 },
    },
    habitats: [
      {
        name: '温带山地竹林',
        realm: 'terrestrial',
        description: '通常位于海拔约 1,200—4,100 米、林下竹类丰富的针阔混交林。',
        isPrimary: true,
      },
    ],
    measurements: {
      length: { min: 1.2, max: 1.9, unit: 'm', note: '头体长' },
      weight: { min: 70, max: 120, unit: 'kg', note: '野生成体常见范围' },
    },
    diet: {
      types: ['herbivore'],
      foods: ['箭竹', '木竹', '竹笋', '少量果实与小型动物'],
      description: '虽属食肉目，却以竹子为绝对主食，每日需长时间取食。',
    },
    activity: ['晨昏活动', '独居'],
    tags: ['中国特有种', '旗舰物种', '山地森林', '竹食性'],
    summary: '生活在中国中西部山地竹林中的标志性熊科动物。',
    description: '大熊猫以黑白相间的毛色和高度特化的竹食习性闻名。保护区网络与栖息地廊道帮助部分种群恢复，但破碎化仍限制基因交流与季节迁移。',
    keyFacts: [
      '腕部扩大的籽骨形成“伪拇指”，能把竹秆稳稳握住。',
      '成体每天可取食约 12—38 千克竹子。',
      '不会真正冬眠，会随季节沿山坡迁移寻找竹类。',
    ],
    threats: ['栖息地破碎化', '基础设施阻隔', '竹林对气候变化的敏感性'],
    conservationActions: ['保护区与国家公园建设', '生态廊道恢复', '野外种群监测与遗传管理'],
    metrics: {
      adultLengthCm: [120, 190],
      adultMassKg: [70, 120],
      lifespanYears: [15, 20],
      elevationM: [1200, 4100],
    },
    featuredStats: [
      { key: 'wild-population', label: '野生数量', value: '约 1,864', unit: '只', note: '中国第四次全国调查结果' },
      { key: 'daily-bamboo', label: '每日食竹', value: '12—38', unit: '千克' },
      { key: 'elevation', label: '活动海拔', value: '1,200—4,100', unit: '米' },
    ],
    media: {
      alt: '竹林中的大熊猫',
      focalPoint: { x: 0.5, y: 0.42 },
    },
    sources: sourcesFor('Ailuropoda melanoleuca'),
    featured: true,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
  {
    id: 'species-panthera-uncia',
    slug: 'snow-leopard',
    names: { zh: '雪豹', en: 'Snow Leopard', aliases: ['草豹', '艾叶豹'] },
    scientificName: 'Panthera uncia',
    taxonomy: animalTaxonomy(
      taxon('Chordata', '脊索动物门'),
      taxon('Mammalia', '哺乳纲'),
      taxon('Carnivora', '食肉目'),
      taxon('Felidae', '猫科'),
      taxon('Panthera', '豹属'),
    ),
    conservation: conservation('VU', 'decreasing', 2016, 'C1'),
    distribution: {
      realms: ['terrestrial'],
      continents: ['亚洲'],
      regions: ['青藏高原', '喜马拉雅山脉', '帕米尔高原', '中亚山地'],
      countries: ['中国', '蒙古', '印度', '尼泊尔', '不丹', '巴基斯坦', '阿富汗', '俄罗斯', '哈萨克斯坦', '吉尔吉斯斯坦', '塔吉克斯坦', '乌兹别克斯坦'],
      range: '分布跨越中亚与南亚十二个国家的高山地带，中国拥有其大部分适生范围。',
      center: { lat: 35, lng: 82 },
    },
    habitats: [
      {
        name: '高山岩地与草甸',
        realm: 'terrestrial',
        description: '偏好陡峭岩壁、碎石坡和高寒草甸组成的开阔山地。',
        isPrimary: true,
      },
    ],
    measurements: {
      length: { min: 90, max: 115, unit: 'cm', note: '不含约 80—100 厘米的长尾' },
      weight: { min: 22, max: 55, unit: 'kg' },
    },
    diet: {
      types: ['carnivore'],
      foods: ['岩羊', '北山羊', '旱獭', '鼠兔', '鸟类'],
      description: '依靠伏击捕食山地有蹄类，也会灵活利用较小猎物。',
    },
    activity: ['晨昏活动', '独居', '领地巡游'],
    tags: ['高山生态系统', '顶级捕食者', '中亚', '旗舰物种'],
    summary: '隐秘穿行于亚洲高山岩脊间的顶级捕食者。',
    description: '雪豹宽大的足掌如天然雪鞋，粗长尾巴既帮助它在峭壁上平衡，也能在休息时保暖。其生存依赖连续的高山栖息地和健康的野生有蹄类种群。',
    keyFacts: [
      '鼻腔宽大，有助于在寒冷、稀薄的空气中呼吸。',
      '一次跳跃可跨越约 15 米，适应断裂的岩石地形。',
      '无法像狮虎那样咆哮，但会发出叫声、嘶声和“呼呼”声。',
    ],
    threats: ['猎物减少', '报复性捕杀', '偷猎与非法贸易', '道路和矿业造成的破碎化'],
    conservationActions: ['跨境保护合作', '社区牲畜保险', '猎物与红外相机监测'],
    metrics: {
      adultLengthCm: [90, 115],
      adultMassKg: [22, 55],
      lifespanYears: [10, 13],
      elevationM: [3000, 5000],
    },
    featuredStats: [
      { key: 'elevation', label: '典型海拔', value: '3,000—5,000', unit: '米' },
      { key: 'tail', label: '尾长', value: '80—100', unit: '厘米' },
      { key: 'range-countries', label: '分布国家', value: '12', unit: '个' },
    ],
    media: {
      image: '/images/fauna-hero.webp',
      alt: '雪山岩壁上的雪豹',
      focalPoint: { x: 0.52, y: 0.43 },
    },
    sources: sourcesFor('Panthera uncia'),
    featured: true,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
  {
    id: 'species-balaenoptera-musculus',
    slug: 'blue-whale',
    names: { zh: '蓝鲸', en: 'Blue Whale', aliases: ['剃刀鲸'] },
    scientificName: 'Balaenoptera musculus',
    taxonomy: animalTaxonomy(
      taxon('Chordata', '脊索动物门'),
      taxon('Mammalia', '哺乳纲'),
      taxon('Cetacea', '鲸目'),
      taxon('Balaenopteridae', '须鲸科'),
      taxon('Balaenoptera', '须鲸属'),
    ),
    conservation: conservation('EN', 'increasing', 2018, 'A1abd'),
    distribution: {
      realms: ['marine'],
      continents: ['亚洲', '非洲', '欧洲', '北美洲', '南美洲', '大洋洲', '南极洲'],
      regions: ['北大西洋', '北太平洋', '南大洋', '印度洋'],
      countries: [],
      range: '除北冰洋核心海域外广布全球，许多种群在高纬摄食区与低纬繁殖区之间迁徙。',
      center: { lat: -45, lng: -100 },
    },
    habitats: [
      {
        name: '远洋与大陆架边缘',
        realm: 'marine',
        description: '季节性聚集在磷虾高密度的涌升流、极锋与大陆架水域。',
        isPrimary: true,
      },
    ],
    measurements: {
      length: { min: 24, max: 30, unit: 'm', note: '南半球个体通常更大' },
      weight: { min: 100, max: 150, unit: 't' },
    },
    diet: {
      types: ['filter-feeder'],
      foods: ['磷虾', '少量桡足类'],
      description: '张开带有喉腹褶的巨口吞入海水，再用鲸须板滤出小型甲壳动物。',
    },
    activity: ['远距离迁徙', '潜水觅食'],
    tags: ['全球海洋', '须鲸', '迁徙物种', '现生最大动物'],
    summary: '地球上已知体型最大的动物，以微小磷虾为主要食物。',
    description: '蓝鲸流线型的蓝灰色身体可达近三十米。商业捕鲸曾使其数量崩溃；全球禁捕后部分种群缓慢恢复，但船撞、水下噪声与气候驱动的食物变化仍是长期压力。',
    keyFacts: [
      '在摄食旺季，一天可摄入数吨磷虾。',
      '低频鸣声能在海水中传播数百甚至上千千米。',
      '喷气可形成高约 9—12 米的垂直雾柱。',
    ],
    threats: ['船舶碰撞', '渔具缠绕', '水下噪声', '气候变化影响磷虾资源'],
    conservationActions: ['国际商业捕鲸禁令', '航线调整与限速', '声学监测和种群调查'],
    metrics: {
      adultLengthCm: [2400, 3000],
      adultMassKg: [100000, 150000],
      lifespanYears: [70, 90],
      maxDiveDepthM: 500,
    },
    featuredStats: [
      { key: 'length', label: '体长', value: '24—30', unit: '米' },
      { key: 'mass', label: '体重', value: '100—150', unit: '吨' },
      { key: 'lifespan', label: '寿命', value: '70—90', unit: '年' },
    ],
    media: {
      alt: '深蓝海水中游弋的蓝鲸',
      focalPoint: { x: 0.55, y: 0.48 },
    },
    sources: sourcesFor('Balaenoptera musculus'),
    featured: true,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
  {
    id: 'species-loxodonta-africana',
    slug: 'african-savanna-elephant',
    names: { zh: '非洲草原象', en: 'African Savanna Elephant', aliases: ['非洲稀树草原象'] },
    scientificName: 'Loxodonta africana',
    taxonomy: animalTaxonomy(
      taxon('Chordata', '脊索动物门'),
      taxon('Mammalia', '哺乳纲'),
      taxon('Proboscidea', '长鼻目'),
      taxon('Elephantidae', '象科'),
      taxon('Loxodonta', '非洲象属'),
    ),
    conservation: conservation('EN', 'decreasing', 2020, 'A2abd'),
    distribution: {
      realms: ['terrestrial', 'freshwater'],
      continents: ['非洲'],
      regions: ['撒哈拉以南非洲', '东非', '南部非洲'],
      countries: ['博茨瓦纳', '坦桑尼亚', '肯尼亚', '津巴布韦', '纳米比亚', '南非'],
      range: '呈斑块状分布于撒哈拉以南的草原、灌丛、林地和湿地景观。',
      center: { lat: -15, lng: 25 },
    },
    habitats: [
      {
        name: '热带稀树草原',
        realm: 'terrestrial',
        description: '在草地、金合欢林地与季节性水源之间长距离移动。',
        isPrimary: true,
      },
      {
        name: '泛洪平原与湿地边缘',
        realm: 'freshwater',
        description: '旱季的重要饮水与取食地。',
      },
    ],
    measurements: {
      height: { min: 2.5, max: 4, unit: 'm', note: '肩高，雄性通常更高' },
      weight: { min: 4, max: 7.5, unit: 't', note: '成年雄性常见范围' },
    },
    diet: {
      types: ['herbivore'],
      foods: ['草', '树叶', '树皮', '嫩枝', '果实'],
      description: '随季节切换食谱，并用长鼻完成采食、饮水和触觉交流。',
    },
    activity: ['昼夜活动', '母系群居', '季节迁移'],
    tags: ['非洲', '生态系统工程师', '大型食草动物', '群居'],
    summary: '现生最大的陆地动物，也是塑造非洲草原景观的生态系统工程师。',
    description: '非洲草原象能推倒树木、开辟林间空地并挖掘水源，为许多物种改变栖息条件。象群由经验丰富的雌象带领，以低频声音和地面振动远距离沟通。',
    keyFacts: [
      '成年个体一天可取食约 100—150 千克植物。',
      '妊娠期接近 22 个月，是陆生哺乳动物中最长之一。',
      '大耳朵拥有丰富血管，可通过扇动帮助散热。',
    ],
    threats: ['象牙偷猎', '栖息地丧失与破碎化', '人象冲突', '干旱加剧'],
    conservationActions: ['反偷猎执法', '跨境迁徙廊道', '社区共管与冲突缓解'],
    metrics: {
      adultMassKg: [4000, 7500],
      lifespanYears: [60, 70],
      topSpeedKph: 40,
    },
    featuredStats: [
      { key: 'height', label: '最大肩高', value: '约 4', unit: '米' },
      { key: 'gestation', label: '妊娠期', value: '约 22', unit: '个月' },
      { key: 'daily-food', label: '每日食量', value: '100—150', unit: '千克' },
    ],
    media: {
      alt: '非洲稀树草原上的草原象群',
      focalPoint: { x: 0.52, y: 0.44 },
    },
    sources: sourcesFor('Loxodonta africana'),
    featured: true,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
  {
    id: 'species-aptenodytes-forsteri',
    slug: 'emperor-penguin',
    names: { zh: '帝企鹅', en: 'Emperor Penguin', aliases: ['皇帝企鹅'] },
    scientificName: 'Aptenodytes forsteri',
    taxonomy: animalTaxonomy(
      taxon('Chordata', '脊索动物门'),
      taxon('Aves', '鸟纲'),
      taxon('Sphenisciformes', '企鹅目'),
      taxon('Spheniscidae', '企鹅科'),
      taxon('Aptenodytes', '王企鹅属'),
    ),
    conservation: conservation('EN', 'decreasing', 2026, 'A3bc'),
    distribution: {
      realms: ['terrestrial', 'marine'],
      continents: ['南极洲'],
      regions: ['南极大陆沿岸', '南大洋'],
      countries: [],
      range: '繁殖地环绕南极大陆，多位于稳定的沿岸海冰；非繁殖期进入南大洋觅食。',
      center: { lat: -74, lng: 20 },
    },
    habitats: [
      {
        name: '南极季节性海冰',
        realm: 'marine',
        description: '需要能维持到幼鸟换羽期的稳定海冰作为繁殖平台。',
        isPrimary: true,
      },
      {
        name: '南大洋开放水域',
        realm: 'marine',
        description: '在冰缘和水下追逐鱼、磷虾与头足类。',
      },
    ],
    measurements: {
      height: { min: 1.1, max: 1.3, unit: 'm' },
      weight: { min: 22, max: 45, unit: 'kg', note: '随繁殖季显著波动' },
    },
    diet: {
      types: ['carnivore', 'piscivore'],
      foods: ['南极银鱼', '磷虾', '鱿鱼'],
      description: '在海冰下和外海潜水捕食，利用流线型身体与鳍状翼推进。',
    },
    activity: ['群居繁殖', '深潜觅食', '季节移动'],
    tags: ['南极', '海冰依赖', '潜水鸟类', '气候变化'],
    summary: '唯一在南极严冬海冰上繁殖、也是体型最大的现生企鹅。',
    description: '帝企鹅在极夜和强风中完成繁殖。雌鸟产卵后赴海觅食，雄鸟把卵托在脚背的育儿袋下，密集成群抵御严寒；海冰过早消失会令尚未长出防水羽毛的幼鸟失去平台。',
    keyFacts: [
      '雄鸟独自孵卵约 65 天，其间通常不进食。',
      '潜水深度可超过 500 米，闭气时间可超过 20 分钟。',
      '群体会不断轮换位置，让外围个体也有机会进入温暖中心。',
    ],
    threats: ['气候变化导致海冰丧失', '食物网变化', '极端天气造成繁殖失败'],
    conservationActions: ['温室气体减排', '繁殖地卫星监测', '南大洋渔业与海洋保护区管理'],
    metrics: {
      adultMassKg: [22, 45],
      lifespanYears: [15, 20],
      maxDiveDepthM: 564,
    },
    featuredStats: [
      { key: 'height', label: '身高', value: '1.1—1.3', unit: '米' },
      { key: 'dive', label: '最深潜水记录', value: '约 564', unit: '米' },
      { key: 'incubation', label: '雄鸟孵卵', value: '约 65', unit: '天' },
    ],
    media: {
      alt: '南极海冰上的帝企鹅群',
      focalPoint: { x: 0.5, y: 0.38 },
    },
    sources: [
      ...sourcesFor('Aptenodytes forsteri'),
      {
        title: 'IUCN 2026：帝企鹅因气候变化升为濒危',
        url: 'https://iucn.org/press-release/202604/emperor-penguin-and-antarctic-fur-seal-now-endangered-due-climate-change-iucn',
        kind: 'conservation',
        accessedAt: CONTENT_DATE,
      },
    ],
    featured: true,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
  {
    id: 'species-rhincodon-typus',
    slug: 'whale-shark',
    names: { zh: '鲸鲨', en: 'Whale Shark', aliases: ['豆腐鲨', '大憨鲨'] },
    scientificName: 'Rhincodon typus',
    taxonomy: animalTaxonomy(
      taxon('Chordata', '脊索动物门'),
      taxon('Chondrichthyes', '软骨鱼纲'),
      taxon('Orectolobiformes', '须鲨目'),
      taxon('Rhincodontidae', '鲸鲨科'),
      taxon('Rhincodon', '鲸鲨属'),
    ),
    conservation: conservation('EN', 'decreasing', 2016, 'A2bd+4bd'),
    distribution: {
      realms: ['marine'],
      continents: ['亚洲', '非洲', '北美洲', '南美洲', '大洋洲'],
      regions: ['印度—太平洋', '大西洋热带海域'],
      countries: ['澳大利亚', '菲律宾', '墨西哥', '马尔代夫'],
      range: '广布约南北纬 30 度之间的暖水海域，并在季节性浮游生物繁盛区聚集。',
      center: { lat: 5, lng: 110 },
    },
    habitats: [
      {
        name: '热带远洋与陆架水域',
        realm: 'marine',
        description: '常出现于上升流、珊瑚产卵场和鱼卵密集的高生产力水域。',
        isPrimary: true,
      },
    ],
    measurements: {
      length: { min: 5.5, max: 10, unit: 'm', note: '常见成体；可靠记录可超过 18 米' },
      weight: { min: 10, max: 20, unit: 't', note: '大型个体估计范围' },
    },
    diet: {
      types: ['filter-feeder'],
      foods: ['浮游动物', '鱼卵', '磷虾', '小型鱼类'],
      description: '通过主动吸水和滤食获取密集的小型猎物，对人类通常温和。',
    },
    activity: ['远距离迁徙', '垂直潜水', '季节性聚集'],
    tags: ['热带海洋', '最大鱼类', '滤食者', '迁徙物种'],
    summary: '现生最大的鱼类，一身白色斑点如同独一无二的指纹。',
    description: '鲸鲨拥有宽阔的端位口，却主要以浮游生物为食。卫星追踪显示它们能跨越洋盆，也会深入千米以下水层；缓慢生长和较晚成熟使种群难以承受捕捞压力。',
    keyFacts: [
      '每条鲸鲨的斑点图案都不同，可用于照片识别。',
      '虽名为“鲸”，实际上是以软骨构成骨骼的鲨鱼。',
      '已有个体被记录潜至约 1,900 米深处。',
    ],
    threats: ['定向和兼捕渔业', '船舶撞击', '塑料污染', '无序旅游干扰'],
    conservationActions: ['国际贸易管制', '聚集地保护与观鲨规范', '卫星标记和照片识别'],
    metrics: {
      adultLengthCm: [550, 1000],
      adultMassKg: [10000, 20000],
      lifespanYears: [70, 100],
      maxDiveDepthM: 1900,
    },
    featuredStats: [
      { key: 'length', label: '常见成体体长', value: '5.5—10', unit: '米' },
      { key: 'record-length', label: '可靠最大记录', value: '约 18.8', unit: '米' },
      { key: 'dive', label: '记录潜深', value: '约 1,900', unit: '米' },
    ],
    media: {
      alt: '热带海面下游动的鲸鲨',
      focalPoint: { x: 0.52, y: 0.46 },
    },
    sources: sourcesFor('Rhincodon typus'),
    featured: true,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
  {
    id: 'species-varanus-komodoensis',
    slug: 'komodo-dragon',
    names: { zh: '科莫多巨蜥', en: 'Komodo Dragon', aliases: ['科莫多龙'] },
    scientificName: 'Varanus komodoensis',
    taxonomy: animalTaxonomy(
      taxon('Chordata', '脊索动物门'),
      taxon('Reptilia', '爬行纲'),
      taxon('Squamata', '有鳞目'),
      taxon('Varanidae', '巨蜥科'),
      taxon('Varanus', '巨蜥属'),
    ),
    conservation: conservation('EN', 'decreasing', 2021, 'B1ab(iii)+2ab(iii)'),
    distribution: {
      realms: ['terrestrial'],
      continents: ['亚洲'],
      regions: ['小巽他群岛'],
      countries: ['印度尼西亚'],
      endemicTo: ['科莫多岛', '林卡岛', '弗洛勒斯岛', '吉利莫堂岛'],
      range: '现生野外种群局限于印度尼西亚小巽他群岛的少数岛屿。',
      center: { lat: -8.56, lng: 119.48 },
    },
    habitats: [
      {
        name: '热带干燥草原与季风林',
        realm: 'terrestrial',
        description: '利用低地稀树草原、灌丛、季风林和季节性溪谷。',
        isPrimary: true,
      },
    ],
    measurements: {
      length: { min: 2, max: 3, unit: 'm' },
      weight: { min: 70, max: 90, unit: 'kg', note: '大型野生雄性' },
    },
    diet: {
      types: ['carnivore'],
      foods: ['帝汶鹿', '野猪', '水牛幼体', '腐肉', '同类幼体'],
      description: '兼具伏击捕猎与食腐习性，以锯齿状牙齿和含毒腺分泌物的咬伤制服猎物。',
    },
    activity: ['日行性', '伏击捕猎', '独居'],
    tags: ['岛屿特有种', '最大蜥蜴', '顶级捕食者', '印度尼西亚'],
    summary: '仅存于印度尼西亚少数岛屿的现生最大蜥蜴。',
    description: '科莫多巨蜥是干燥岛屿生态系统的顶级捕食者。灵敏的犁鼻器让它能循气味寻找远处猎物；海平面上升、栖息地缩减和猎物变化使狭窄分布区面临风险。',
    keyFacts: [
      '分叉舌把气味颗粒送入犁鼻器，可追踪数千米外的腐肉。',
      '幼蜥常爬到树上，避开会捕食它们的成年同类。',
      '短距离奔跑速度可接近每小时 20 千米。',
    ],
    threats: ['栖息地缩减', '海平面上升', '猎物减少', '人类活动与火灾'],
    conservationActions: ['科莫多国家公园保护', '猎物与种群监测', '限制栖息地开发'],
    metrics: {
      adultLengthCm: [200, 300],
      adultMassKg: [70, 90],
      lifespanYears: [20, 30],
      topSpeedKph: 20,
    },
    featuredStats: [
      { key: 'length', label: '最大体长', value: '约 3', unit: '米' },
      { key: 'speed', label: '短跑速度', value: '约 20', unit: '千米/时' },
      { key: 'islands', label: '现存主要岛屿', value: '4', unit: '座' },
    ],
    media: {
      alt: '干燥草地上的科莫多巨蜥',
      focalPoint: { x: 0.5, y: 0.5 },
    },
    sources: sourcesFor('Varanus komodoensis'),
    featured: true,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
  {
    id: 'species-gavialis-gangeticus',
    slug: 'gharial',
    names: { zh: '恒河鳄', en: 'Gharial', aliases: ['食鱼鳄', '长吻鳄'] },
    scientificName: 'Gavialis gangeticus',
    taxonomy: animalTaxonomy(
      taxon('Chordata', '脊索动物门'),
      taxon('Reptilia', '爬行纲'),
      taxon('Crocodylia', '鳄目'),
      taxon('Gavialidae', '长吻鳄科'),
      taxon('Gavialis', '恒河鳄属'),
    ),
    conservation: conservation('CR', 'decreasing', 2017, 'A2bcde+4bcde'),
    distribution: {
      realms: ['freshwater'],
      continents: ['亚洲'],
      regions: ['印度河—恒河平原'],
      countries: ['印度', '尼泊尔'],
      range: '稳定繁殖种群现主要集中在印度和尼泊尔少数未被截断的大河河段。',
      center: { lat: 26.6, lng: 80.6 },
    },
    habitats: [
      {
        name: '大型低地河流',
        realm: 'freshwater',
        description: '需要深水、缓急相间的河道以及可供晒背和筑巢的宽阔沙洲。',
        isPrimary: true,
      },
    ],
    measurements: {
      length: { min: 3.5, max: 6, unit: 'm', note: '雄性通常明显大于雌性' },
      weight: { min: 160, max: 250, unit: 'kg', note: '大型成体估计范围' },
    },
    diet: {
      types: ['carnivore', 'piscivore'],
      foods: ['淡水鱼', '甲壳类', '两栖动物'],
      description: '极狭长的吻部减少水阻，交错尖牙尤其适合迅速夹住滑溜的鱼。',
    },
    activity: ['水栖', '日间晒背', '季节性筑巢'],
    tags: ['南亚河流', '极危', '食鱼特化', '淡水旗舰物种'],
    summary: '长吻高度特化、几乎完全水栖的南亚大型鳄类。',
    description: '恒河鳄的腿不适合在陆地高抬行走，却是灵活的游泳者。成年雄性吻端长有壶状突起“ghara”，既能放大求偶声，也会在吐气时产生气泡。',
    keyFacts: [
      '成年雄性的吻端会形成该物种独有的壶状突起。',
      '雌鳄一次可在沙岸产下约 30—60 枚卵。',
      '它们通常避开人类，狭窄口鼻不适于捕捉大型陆生猎物。',
    ],
    threats: ['水坝与河道改造', '渔网误捕', '采砂破坏巢址', '鱼类资源下降'],
    conservationActions: ['河流保护区', '人工繁育放归', '巢址巡护与渔业协作'],
    metrics: {
      adultLengthCm: [350, 600],
      adultMassKg: [160, 250],
      lifespanYears: [40, 60],
    },
    featuredStats: [
      { key: 'length', label: '雄性体长', value: '可达 6', unit: '米' },
      { key: 'clutch', label: '每窝卵数', value: '约 30—60', unit: '枚' },
      { key: 'range', label: '主要现存国家', value: '2', unit: '个' },
    ],
    media: {
      alt: '河岸沙洲上晒太阳的恒河鳄',
      focalPoint: { x: 0.55, y: 0.52 },
    },
    sources: sourcesFor('Gavialis gangeticus'),
    featured: false,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
  {
    id: 'species-ambystoma-mexicanum',
    slug: 'axolotl',
    names: { zh: '墨西哥钝口螈', en: 'Axolotl', aliases: ['美西螈', '六角恐龙'] },
    scientificName: 'Ambystoma mexicanum',
    taxonomy: animalTaxonomy(
      taxon('Chordata', '脊索动物门'),
      taxon('Amphibia', '两栖纲'),
      taxon('Caudata', '有尾目'),
      taxon('Ambystomatidae', '钝口螈科'),
      taxon('Ambystoma', '钝口螈属'),
    ),
    conservation: conservation('CR', 'decreasing', 2019, 'A2ace; B2ab(iii,v)'),
    distribution: {
      realms: ['freshwater'],
      continents: ['北美洲'],
      regions: ['墨西哥谷地', '霍奇米尔科湿地'],
      countries: ['墨西哥'],
      endemicTo: ['墨西哥城霍奇米尔科运河系统'],
      range: '天然分布已收缩至墨西哥城南部霍奇米尔科湖残存的运河与湿地。',
      center: { lat: 19.27, lng: -99.1 },
    },
    habitats: [
      {
        name: '高原湖泊与运河湿地',
        realm: 'freshwater',
        description: '依赖水温较凉、水生植物丰富且水质良好的缓流水体。',
        isPrimary: true,
      },
    ],
    measurements: {
      length: { min: 15, max: 30, unit: 'cm' },
      weight: { min: 60, max: 200, unit: 'g', note: '成体常见范围' },
    },
    diet: {
      types: ['carnivore'],
      foods: ['水生昆虫幼虫', '蠕虫', '小型甲壳类', '小鱼'],
      description: '伏在水底，以快速吸水产生的负压将小型猎物吸入口中。',
    },
    activity: ['夜行性', '终生水栖', '伏击捕食'],
    tags: ['墨西哥特有种', '极危', '终生幼态', '再生能力'],
    summary: '终生保留外鳃与幼体形态、拥有非凡再生能力的水栖蝾螈。',
    description: '墨西哥钝口螈通常不经历陆栖成体变态，这种现象称为幼态延续。实验室种群很常见，但野外种群仅存于高度城市化的霍奇米尔科湿地，两者的保护意义不能混同。',
    keyFacts: [
      '可再生四肢、尾部以及部分心脏和神经组织，通常不形成明显瘢痕。',
      '羽毛状外鳃终生保留，同时也能用肺和皮肤交换气体。',
      '宠物中常见的粉白色型并不代表野外常见体色，野生个体多为深褐斑驳。',
    ],
    threats: ['水污染与城市化', '栖息地萎缩', '罗非鱼和鲤鱼等外来种', '小种群隔离'],
    conservationActions: ['湿地与传统浮田修复', '设置外来鱼隔离的庇护水道', '野外监测和保育繁殖'],
    metrics: {
      adultLengthCm: [15, 30],
      adultMassKg: [0.06, 0.2],
      lifespanYears: [10, 15],
    },
    featuredStats: [
      { key: 'range', label: '天然分布', value: '仅霍奇米尔科', unit: '湿地' },
      { key: 'length', label: '体长', value: '15—30', unit: '厘米' },
      { key: 'lifespan', label: '寿命', value: '10—15', unit: '年' },
    ],
    media: {
      alt: '水草间的野生深色墨西哥钝口螈',
      focalPoint: { x: 0.53, y: 0.48 },
    },
    sources: sourcesFor('Ambystoma mexicanum'),
    featured: true,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
  {
    id: 'species-phyllobates-terribilis',
    slug: 'golden-poison-frog',
    names: { zh: '金色箭毒蛙', en: 'Golden Poison Frog', aliases: ['黄金箭毒蛙'] },
    scientificName: 'Phyllobates terribilis',
    taxonomy: animalTaxonomy(
      taxon('Chordata', '脊索动物门'),
      taxon('Amphibia', '两栖纲'),
      taxon('Anura', '无尾目'),
      taxon('Dendrobatidae', '箭毒蛙科'),
      taxon('Phyllobates', '叶毒蛙属'),
    ),
    conservation: conservation('EN', 'decreasing', 2018, 'B1ab(iii)+2ab(iii)'),
    distribution: {
      realms: ['terrestrial', 'freshwater'],
      continents: ['南美洲'],
      regions: ['哥伦比亚太平洋低地'],
      countries: ['哥伦比亚'],
      endemicTo: ['哥伦比亚考卡省与相邻太平洋低地'],
      range: '天然分布范围狭窄，仅见于哥伦比亚西部潮湿的低地雨林。',
      center: { lat: 3.1, lng: -77.1 },
    },
    habitats: [
      {
        name: '潮湿低地热带雨林',
        realm: 'terrestrial',
        description: '活动于高降雨雨林的落叶层，繁殖离不开林下小水体。',
        isPrimary: true,
      },
    ],
    measurements: {
      length: { min: 45, max: 55, unit: 'mm', note: '雌性通常稍大' },
      weight: { min: 20, max: 30, unit: 'g' },
    },
    diet: {
      types: ['insectivore'],
      foods: ['蚂蚁', '螨类', '甲虫', '白蚁'],
      description: '在林地表面捕食微小节肢动物；野外毒素可能来自食物链中的特定猎物。',
    },
    activity: ['日行性', '地栖', '领域鸣叫'],
    tags: ['哥伦比亚特有种', '警戒色', '热带雨林', '两栖动物'],
    summary: '以明亮警戒色和强效皮肤生物碱闻名的哥伦比亚雨林蛙。',
    description: '金色箭毒蛙的黄色、橙色或浅绿色体色向捕食者发出警告。人工饲养个体因缺少野外食物来源通常不具同等毒性；森林退化和采矿使其狭小栖息地持续承压。',
    keyFacts: [
      '皮肤中的主要毒素为蛙毒素类生物碱，可干扰神经细胞的钠通道。',
      '亲蛙会把孵化的蝌蚪背到合适水体中。',
      '人工饲养并改变食谱后，皮肤毒性会显著减弱或消失。',
    ],
    threats: ['雨林砍伐', '非法采矿与水污染', '农业扩张', '狭窄分布区'],
    conservationActions: ['关键栖息地保护', '社区雨林管理', '合法保育繁殖以减少野外采集'],
    metrics: {
      adultLengthCm: [4.5, 5.5],
      adultMassKg: [0.02, 0.03],
      lifespanYears: [5, 10],
      elevationM: [0, 200],
    },
    featuredStats: [
      { key: 'length', label: '体长', value: '4.5—5.5', unit: '厘米' },
      { key: 'range', label: '分布', value: '哥伦比亚西部', unit: '低地雨林' },
      { key: 'activity', label: '活动节律', value: '日行性' },
    ],
    media: {
      alt: '雨林落叶上的金色箭毒蛙',
      focalPoint: { x: 0.5, y: 0.45 },
    },
    sources: sourcesFor('Phyllobates terribilis'),
    featured: false,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
  {
    id: 'species-bombus-affinis',
    slug: 'rusty-patched-bumble-bee',
    names: { zh: '锈斑熊蜂', en: 'Rusty-patched Bumble Bee', aliases: ['锈补丁熊蜂'] },
    scientificName: 'Bombus affinis',
    taxonomy: animalTaxonomy(
      taxon('Arthropoda', '节肢动物门'),
      taxon('Insecta', '昆虫纲'),
      taxon('Hymenoptera', '膜翅目'),
      taxon('Apidae', '蜜蜂科'),
      taxon('Bombus', '熊蜂属'),
    ),
    conservation: conservation('CR', 'decreasing', 2014, 'A2abce'),
    distribution: {
      realms: ['terrestrial'],
      continents: ['北美洲'],
      regions: ['北美东部温带区', '五大湖地区'],
      countries: ['美国', '加拿大'],
      range: '曾广布北美东部，如今主要残存在上中西部及少数东北部地点。',
      center: { lat: 43, lng: -89 },
    },
    habitats: [
      {
        name: '温带草地与林缘',
        realm: 'terrestrial',
        description: '需要春至秋连续开花的植物、地下巢穴和未受扰动的越冬土壤。',
        isPrimary: true,
      },
      {
        name: '城市花园与农田边缘',
        realm: 'terrestrial',
        description: '管理得当的本地植物花园可提供重要补充生境。',
      },
    ],
    measurements: {
      length: { min: 10, max: 22, unit: 'mm', note: '工蜂到蜂王的范围' },
    },
    diet: {
      types: ['nectarivore', 'herbivore'],
      foods: ['多种本地野花的花蜜', '花粉'],
      description: '访花取食花蜜和花粉，并为多种野生植物与农作物完成授粉。',
    },
    activity: ['日行性', '社会性群居', '季节性蜂群'],
    tags: ['传粉者', '北美草地', '社会性昆虫', '极危'],
    summary: '腹部带锈色斑块、曾经常见却在北美东部急剧衰退的传粉昆虫。',
    description: '锈斑熊蜂的蜂群每年重建：只有新蜂王越冬，翌春寻找巢址。病原体、农药、花源不足和气候变化共同导致其分布大幅收缩。',
    keyFacts: [
      '只有工蜂和雄蜂腹部通常显出名称所指的锈色斑块。',
      '通过高频振动释放花粉的“震动授粉”，能服务番茄等植物。',
      '美国鱼类及野生动物管理局估计其历史分布区已缩减约 87%。',
    ],
    threats: ['病原体传播', '杀虫剂暴露', '花源和巢址丧失', '气候变化'],
    conservationActions: ['种植连续花期的本地植物', '减少农药使用', '保护地下巢址与蜂王越冬地'],
    metrics: {
      adultLengthCm: [1, 2.2],
      lifespanYears: [0.08, 1],
    },
    featuredStats: [
      { key: 'range-loss', label: '历史分布区缩减', value: '约 87', unit: '%' },
      { key: 'length', label: '体长', value: '10—22', unit: '毫米' },
      { key: 'cycle', label: '蜂群周期', value: '一年一代' },
    ],
    media: {
      alt: '在花朵上采集花粉的锈斑熊蜂',
      focalPoint: { x: 0.52, y: 0.46 },
    },
    sources: [
      ...sourcesFor('Bombus affinis'),
      {
        title: 'U.S. Fish & Wildlife Service — Rusty Patched Bumble Bee',
        url: 'https://www.fws.gov/species/rusty-patched-bumble-bee-bombus-affinis',
        kind: 'conservation',
        accessedAt: CONTENT_DATE,
      },
    ],
    featured: false,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
  {
    id: 'species-acropora-cervicornis',
    slug: 'staghorn-coral',
    names: { zh: '鹿角珊瑚', en: 'Staghorn Coral', aliases: ['麋角珊瑚'] },
    scientificName: 'Acropora cervicornis',
    taxonomy: animalTaxonomy(
      taxon('Cnidaria', '刺胞动物门'),
      taxon('Anthozoa', '珊瑚纲'),
      taxon('Scleractinia', '石珊瑚目'),
      taxon('Acroporidae', '轴孔珊瑚科'),
      taxon('Acropora', '轴孔珊瑚属'),
    ),
    conservation: conservation('CR', 'decreasing', 2021, 'A2ace'),
    distribution: {
      realms: ['marine'],
      continents: ['北美洲', '南美洲'],
      regions: ['加勒比海', '佛罗里达礁岛群', '巴哈马'],
      countries: ['美国', '巴哈马', '古巴', '伯利兹', '牙买加', '多米尼加共和国'],
      range: '分布于加勒比海及邻近西大西洋温暖浅海，过去曾形成广阔而密集的枝状礁区。',
      center: { lat: 20, lng: -78 },
    },
    habitats: [
      {
        name: '清澈浅水珊瑚礁',
        realm: 'marine',
        description: '多生长在光照充足、水流良好、约 1—30 米深的前礁与礁坪。',
        isPrimary: true,
      },
    ],
    measurements: {
      length: { min: 0.5, max: 2, unit: 'm', note: '珊瑚群体横向尺度，而非单个珊瑚虫' },
    },
    diet: {
      types: ['filter-feeder'],
      foods: ['共生虫黄藻提供的光合产物', '浮游生物', '溶解有机物'],
      description: '主要能量来自组织内共生藻的光合作用，珊瑚虫也会伸出触手捕捉浮游生物。',
    },
    activity: ['固着群体', '夜间伸展触手', '无性断枝繁殖'],
    tags: ['加勒比海', '造礁珊瑚', '共生', '极危'],
    summary: '枝条如鹿角般伸展、为加勒比珊瑚礁营造三维庇护空间的造礁动物。',
    description: '鹿角珊瑚生长较快，断裂枝条也能附着形成新群体，因此是礁体恢复的重要物种。白带病、海洋热浪和飓风已使其覆盖率较历史水平大幅下降。',
    keyFacts: [
      '健康群体每年可延伸约 10—20 厘米，是生长较快的加勒比造礁珊瑚。',
      '复杂枝杈为幼鱼和无脊椎动物提供藏身空间。',
      '水温异常会破坏珊瑚与共生藻关系，引发“白化”。',
    ],
    threats: ['海洋热浪与白化', '白带病等疾病', '风暴破坏', '污染和沉积物'],
    conservationActions: ['珊瑚苗圃与断枝移植', '改善沿岸水质', '疾病监测', '减缓气候变化'],
    metrics: {
      adultLengthCm: [50, 200],
      lifespanYears: [10, 100],
    },
    featuredStats: [
      { key: 'growth', label: '年生长', value: '约 10—20', unit: '厘米' },
      { key: 'depth', label: '常见水深', value: '1—30', unit: '米' },
      { key: 'form', label: '生命形态', value: '珊瑚虫群体' },
    ],
    media: {
      alt: '加勒比浅海中的鹿角珊瑚群体',
      focalPoint: { x: 0.5, y: 0.48 },
    },
    sources: [
      ...sourcesFor('Acropora cervicornis'),
      {
        title: 'NOAA Fisheries — Staghorn Coral',
        url: 'https://www.fisheries.noaa.gov/species/staghorn-coral',
        kind: 'ecology',
        accessedAt: CONTENT_DATE,
      },
    ],
    featured: true,
    publishedAt: CONTENT_DATE,
    updatedAt: CONTENT_DATE,
  },
] as const satisfies readonly Species[];

export const speciesBySlug: Readonly<Record<string, Species>> = Object.fromEntries(
  species.map((item) => [item.slug, item]),
);

export const featuredSpecies: readonly Species[] = species.filter((item) => item.featured);

export const getSpeciesBySlug = (slug: string): Species | undefined => speciesBySlug[slug];

export const getSpeciesById = (id: string): Species | undefined =>
  species.find((item) => item.id === id);
